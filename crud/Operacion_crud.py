...
class OperacionCRUD:
    ...
    def crear_operacion(
        ...
        if id_cuenta_origen:
            cuenta_origen = self.db.query(Cuenta).filter(
                Cuenta.id_cuenta == id_cuenta_origen
            ).first()
            if not cuenta_origen:
                raise ValueError("La cuenta origen no existe")

        if id_cuenta_destino:
            cuenta_destino = self.db.query(Cuenta).filter(
                Cuenta.id_cuenta == id_cuenta_destino
            ).first()
